...
GraphVertex* CloneHelper(unordered_map<int, GraphVertex*>& seen,
                         GraphVertex* curr) {
    auto clone = new GraphVertex{};
    clone->label = curr->label;
    seen[clone->label] = clone;
    for (auto edge : curr->edges) {
        if (seen.find(edge->label) == seen.end()) {
            auto temp = CloneHelper(seen, edge);
            seen[edge->label] = temp;
        }
        auto neighbor = seen[edge->label];
        clone->edges.push_back(neighbor);
    }
    return clone;
}
...
GraphVertex* CloneGraph(GraphVertex* graph) {
    ...
    auto seen = unordered_map<int, GraphVertex*>{};
    return CloneHelper(seen, graph);
}
